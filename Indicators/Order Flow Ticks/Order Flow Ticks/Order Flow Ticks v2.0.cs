    ...
    public class OrderFlowTicksV20 : Indicator
    {
        ...
        private void OrderFlow(int iStart, bool isLookback = false)
        {
            ...
            if (Chart.TimeFrame.ToString().Contains("Renko") && ShowWicks) {
                ...
                var nextOpenTime = (iStart + 1 < Bars.Count) ? Bars.OpenTimes[iStart + 1] : TicksOHLC.LastBar.OpenTime;
            ...
            for (int i = 0; i < Segments.Count; i++)
            {
                ...
                */
                ...
                // Any Volume Mode
                double maxLength = 0;
                if ((!IsLastBar || isBarClosed) && iStart + 1 < Bars.Count)
                    maxLength = Bars[iStart + 1].OpenTime.Subtract(Bars[iStart].OpenTime).TotalMilliseconds;
                else
                    maxLength = (iStart > 0) ? Bars[iStart].OpenTime.Subtract(Bars[iStart - 1].OpenTime).TotalMilliseconds : 60000;
                ...
                void DrawRectangle_Normal(int currentVolume, int maxVolume, bool profileInMiddle = false)
                {
                    ...
                    bool gapWeekend = (iStart + 1 < Bars.Count) && xBar.DayOfWeek == DayOfWeek.Friday && Bars.OpenTimes[iStart + 1].DayOfWeek == DayOfWeek.Sunday;
                    if (gapWeekend) {
                        x1 = xBar;
                        x2 = x1.AddMilliseconds(dynLength);
                    }
