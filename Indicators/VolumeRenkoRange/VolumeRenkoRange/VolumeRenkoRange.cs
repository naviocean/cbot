...
namespace cAlgo
{
    ...
    public class VolumeRenkoRange : Indicator
    {
        ...
        private PopupNotification asyncTickPopup = null;
        ...
        private void Filters(int index, bool isUp, double volume)
        {
            switch (VolumeColoring_Input)
            {
                ...
                case VolumeColoring_Data.Heatmap:
                    ...
                    switch (VolumeFilter_Input)
                    {
                        case VolumeFilter_Data.MA:
                            strengthVolume = maVol.Result[index] != 0 ? volume / maVol.Result[index] : 0;
                            break;
                        case VolumeFilter_Data.Standard_Deviation:
                            strengthVolume = stdDev.Result[index] != 0 ? volume / stdDev.Result[index] : 0;
                            break;
                        case VolumeFilter_Data.Both:
                            strengthVolume = stdDev.Result[index] != 0 ? (volume - maVol.Result[index]) / stdDev.Result[index] : 0;
                            break;
                        case VolumeFilter_Data.Normalized_Emphasized:
                            double Normalization()
                            {
                                ...
                                double normalizedValue = avg != 0 ? VolumeSeries[index] / avg : 0;
                    ...
                    _ = strengthVolume < lowest ? heatmapSeries(HeatmapSwitch.Lowest) :
                        ...
                        strengthVolume >= ultra ? heatmapSeries(HeatmapSwitch.Ultra) : heatmapSeries(HeatmapSwitch.Lowest);
                ...
                case VolumeColoring_Data.Fading:
                    ...
                    else if (volume < VolumeSeries[index - 1])
                    {
        ...
        private int VolumeTick(int index)
        {
            ...
            DateTime endTime = (index + 1 < Bars.Count) ? Bars.OpenTimes[index + 1] : TicksOHLC.LastBar.OpenTime;
            ...
            if (IsLastBar) {
                while (startIndex < TicksOHLC.Count && TicksOHLC.OpenTimes[startIndex] < startTime)
                    startIndex++;
        ...
        private void Recalculate() {
            int firstTickIndex = Bars.OpenTimes.GetIndexByTime(TicksOHLC.OpenTimes.FirstOrDefault());
            int startIndex = Math.Max(0, firstTickIndex);
        ...
        // *********** VOLUME RENKO/RANGE ***********
        /*
            Original source code by srlcarlg (https://ctrader.com/algos/indicators/show/3045)
        ...
        */
        private void VolumeInitialize(bool onlyDate = false)
        {
            ...
            if (firstTickTime >= fromDateTime) {
                ...
                if (notifyIsMinimal && progressPopup != null)
        ...
        private void DrawStartVolumeLine() {
            try {
                ...
                int barIdx = Bars.OpenTimes.GetIndexByTime(firstTickDate);
                if (barIdx >= 0) {
                    ChartText textInfo = Chart.DrawText("VolumeStartText", "Tick Volume Data \n ends here", firstTickDate, Bars.HighPrices[barIdx], Color.Red);
        ...
        private void DrawFromDateLine() {
            try {
                ...
                lineInfo.LineStyle = LineStyle.Lines;
                int barIdx = Bars.OpenTimes.GetIndexByTime(fromDateTime);
                if (barIdx >= 0) {
                    ChartText textInfo = Chart.DrawText("FromDateText", "Target Tick Data", fromDateTime, Bars.HighPrices[barIdx], Color.Yellow);
                ...
                if (!loadingTicksComplete)
                {
                    TicksOHLC.LoadMoreHistoryAsync((_) =>
                    {
                        ...
                        if (currentDate <= fromDateTime)
                        {
                            ...
                            if (asyncTickPopup != null && asyncTickPopup.State != PopupNotificationState.Success)
